...
async function run() {
  try {
    ...
    const { data: service, error: serviceErr } = await supabase
      .from('services')
      .insert({ name: 'Teste Cortes', duration: 30, price: 50 })
      .select('*')
      .single();
    ...
    const { data: professional, error: profErr } = await supabase
      .from('professionals')
      .insert({ name: 'Teste Profissional', email: 'prof@test.com', specialties: ['corte'] })
      .select('*')
      .single();
    ...
    const { data: client, error: clientErr } = await supabase
      .from('clients')
      .insert({ name: 'Cliente Teste', phone: '11999999999', email: 'cliente@test.com', consent_lgpd: true })
      .select('*')
      .single();
